...
class User:
    ...
    def update_rank(self):
        if self.progress >= 100:
            diff = self.progress // 100 + self.levels.index(self.rank)
            self.rank = self.levels[diff]
            self.progress %= 100
        if self.rank == 8:
            self.progress = 0
    ...
    def inc_progress(self, level):
        if level not in self.levels:
            raise Exception("level not in range")
        d = self.levels.index(level) - self.levels.index(self.rank)
        if d == 0:
            self.progress += 3
        elif d == -1:
            self.progress += 1
